...
function addExportButtonsAlwaysShow() {
    const page = window.location.pathname.split("/").pop() || "index.html";

    const configs = {
        "admin-students.html": {
            id: "adminExportStudentsCsvBtn",
            label: "Export Students CSV",
            action: exportStudentsCSV
        },
        "admin-parents.html": {
            id: "adminExportParentsCsvBtn",
            label: "Export Parents CSV",
            action: exportParentsCSV
        },
        "admin-payments.html": {
            id: "adminExportPaymentsCsvBtn",
    ...
    wrapper.className = "export-csv-bar";
    ...
    const firstPanel = document.querySelector(".panel-card");
    ...
    if (adminMain) {
        adminMain.insertBefore(wrapper, adminMain.firstChild);
        return;
    }
...
function startExportButtonFix() {
    addExportButtonsAlwaysShow();

    setTimeout(addExportButtonsAlwaysShow, 300);
    setTimeout(addExportButtonsAlwaysShow, 1000);
    setTimeout(addExportButtonsAlwaysShow, 2000);
}

document.addEventListener("DOMContentLoaded", startExportButtonFix);
window.addEventListener("load", startExportButtonFix);

// MUTAHUS_FIX_STEP18_EXPORT_BUTTONS_ALWAYS_SHOW
